...
Model::Model(Model&& rv) noexcept :m_name(std::move(rv.m_name))
, m_id(0)
...
void Model::addMesh(const aiScene* scene, const aiMesh* mesh, aiMatrix4x4 transform, MeshLoadOption options) {
...
Mesh* Model::addMesh(const std::vector<Vertex_t>& vertices, const std::vector<Index_t>& indices, PrimitiveType pt) {
...
Mesh* Model::addMesh(std::vector<Vertex_t>&& vertices, std::vector<Index_t>&& indices, PrimitiveType pt) {
	auto mesh = std::make_unique<Mesh>();
...
std::vector<const Mesh*> Model::getMeshes() const {
...
std::shared_ptr<Material> Model::embededMaterialForMesh(const Mesh* mesh) {
...
void Model::loadGeometry(const aiMesh* aMesh, std::vector<Vertex_t>& vertices, std::vector<Index_t>& indices, PrimitiveType& pt) {
	vertices.clear();
...
std::shared_ptr<Material> Model::loadMaterial(const aiScene* aScene, const aiMesh* aMesh) {
...
std::shared_ptr<Texture> Model::loadTexture(const std::string& name) {
...
std::ostream& operator << (std::ostream& o, const Model& meshGrp) {
	...
	o << " meshes(cout:" << meshGrp.meshCount() << ")";
	for (size_t i = 0; i < meshGrp.meshCount(); i++) {
		o << " " << *meshGrp.meshAt(i) << "\n";
	}
	o << "}";
	return o;
