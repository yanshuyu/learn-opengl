...
DeferredRendererApp::DeferredRendererApp(const std::string& t, int w, int h) :GLApplication(t, w, h)
, m_scene(nullptr)
, m_renderer(nullptr) {

}

bool DeferredRendererApp::initailize() {
	if (!__super::initailize())
		return false;
	
	m_scene = std::make_unique<Scene>(glm::vec2(m_wndWidth, m_wndHeight), "deferred_rendering_scene");
	m_renderer = std::unique_ptr<Renderer>(new Renderer(m_wndWidth, m_wndHeight));

	m_renderer->setRenderMode(Renderer::Mode::Deferred);
...
void DeferredRendererApp::update(double dt) {
...
void DeferredRendererApp::render() {
	m_renderer->renderScene(m_scene.get());
}

void DeferredRendererApp::onWindowResized(int width, int height) {
